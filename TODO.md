...
* ~~**HACS version display: create GitHub releases from tags.**~~ **Done.** Now published from GitLab CI: the `release:github` job (`dagger call publish-github-releases`) scans `v*` tags, waits for the push mirror to carry the tag to GitHub, verifies the tag resolves to the same commit GitLab has, and publishes any tag missing a release — severity-grouped notes, version-bump and merge commits filtered out. `release:audit` re-checks on the scheduled pipeline and fails on any gap. Idempotent and self-healing. This is the third design: an in-`tag:auto` poll-and-release attempt never produced a release, a `push: tags` Actions workflow did not reliably fire on mirror pushes, and a scan-on-schedule Actions workflow inherited the same unreliable `push` trigger as its fast path (it fired for v1.1.1, silently did not for v1.2.0). The lesson each time was the same, so the release no longer depends on GitHub triggering anything: GitLab, which already owns tagging, calls the GitHub API itself and fails loudly when it cannot. Spec at `docs/superpowers/specs/2026-04-17-github-release-publishing-design.md` (describes the superseded Actions design).
