...
async def test_lighting_raise_from_scene_steps_on_lights(
    ...
    hass.states.async_set(
        "light.network_room_overhead_1", "on", {"brightness": 128}
    )
    hass.states.async_set(
        "light.network_room_overhead_2", "on", {"brightness": 128}
    )
    ...
    stepped_ids = {
        c.data.get("entity_id") for c in on_calls
...
async def test_lighting_raise_only_dims_currently_on_lights(
    ...
    stepped_ids = {
        c.data.get("entity_id") for c in _light_turn_on_calls(service_calls)
        if c.data.get("brightness_step_pct", 0) != 0
    }
