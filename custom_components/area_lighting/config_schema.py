...
AREA_SCHEMA = vol.Schema({
    ...
    vol.Optional("lutron_remotes", default=[]): vol.All(
        cv.ensure_list, [LUTRON_REMOTE_SCHEMA]
    ),
    vol.Optional("motion_light_motion_sensor_ids"): vol.All(
    ...
    vol.Optional("occupancy_light_sensor_ids"): vol.All(
        cv.ensure_list, [cv.entity_id]
    ),
...
def parse_config(raw: dict) -> AreaLightingConfig:
    ...
    for area_raw in raw.get("areas", []):
        ...
        lights = [
            LightConfig(
                id=l["id"],
                circadian_switch=l.get("circadian_switch"),
                circadian_type=l.get("circadian_type"),
                roles=l.get("roles", []),
                scenes=l.get("scenes", []),
                members=l.get("members", []),
            )
            for l in area_raw.get("lights", [])
        ]
        ...
        light_clusters = [
            ...
            for l in area_raw.get("light_clusters", [])
        ]
        ...
        for lm_raw in area_raw.get("linked_motion", []):
            ...
            linked_motion.append(LinkedMotionConfig(
